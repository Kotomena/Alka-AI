...
/*
 * maskCIN — Masque le CIN dans la liste du dashboard.
 * Garde les 3 premiers et 3 derniers caractères visibles.
 * ex : "101 234 567 890" → "101 ••• ••• 890"
 *      "12345678"        → "123••678"
 */
function maskCIN(cin) {
  if (!cin) return "—";
  var s = String(cin).trim();
  if (s.length <= 6) return "•".repeat(s.length);
  var debut  = s.slice(0, 3);
  var fin    = s.slice(-3);
  var milieu = s.slice(3, -3).replace(/[^\s]/g, "•");
...
async function loadUsersList() {
  ...
  box.innerHTML =
    ...
    "<th>Nom</th><th>Pays</th><th>Photo</th>" +
